...
class File
{
    ...
    public static function create($path, $name)
    {
        ...
        if (file_exists($$filename)) {
            $di->getShared('flash')->error('文件已经存在，创建失败');
        } else {
            if (file_exists($path)) {
                touch($name);
                rename($name, $filename);
                $di->getShared('flash')->error('文件创建失败');
                return true;
            } else {
                $di->getShared('flash')->error('创建失败，创建父级目录失败');
            }
        }
        return false;
    }
    ...
    public static function writeFile($path, $name, $content)
    {
        ...
        if (file_put_contents(ROOT_DIR . $filename, $content)) {
            return true;
        } else {
            return false;
        }
