...
class EntityModel extends Model
{
    ...
    {
        $handleFormInfo = Config::get($this->_module . '.' . $this->_entityId . 'HandleForm');
    ...
    public function deleteEntity()
    {
        ...
        foreach ($this->_fields as $fieldKey => $field) {
            if (isset($field['addition']) && $field['addition'] == true) {
                if (!isset($field['model'])) {
                    $modelName = ucfirst($this->_entityId) . 'Field' . ucfirst($fieldKey);
                    $field['model'] = '\Models\\' . $modelName;
                }

                $model = $field['model'];
                $fieldModel = $model::findByEid($this->id);
                foreach ($fieldModel as $f) {
                    if (!$f->delete()) {
                        $state = false;
                        break;
                    }
        ...
        if ($state === true) {
            ...
            foreach ($f->getMessages() as $message) {
                $error .= '【' . $field['label'] . '】删除失败' . $message . "<br>";
            }
            $this->getDI()->getFlash()->error('删除失败<br>' . $error);
            $db->rollback();
        }
    }
